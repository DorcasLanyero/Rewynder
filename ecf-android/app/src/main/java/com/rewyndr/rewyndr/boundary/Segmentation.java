...
public class Segmentation {
    ...
    private int width;
    private int height;
    private ArrayList<Point> convexHullPointList;
    private int touchPointX;
    private int touchPointY;
    private boolean touchLocationSet = false;
    ...
    public void setTouchLocation(int touchPointX, int touchPointY) {
        ...
        this.touchLocationSet = true;
    }

    public boolean hasBeenTouched() {
        return this.touchLocationSet;
    }
    ...
    public void segmentation(int threshold) {
        ...
        // Convex hull
        convexHullPointList = findConvexHull(borderPointList);
     ...
     */
    private void normalizePointList() {
        int offset = 10;

        // Log.d("POINT LIST", Integer.toString(convexHullPointList.size()));
        for (Point point : convexHullPointList) {
    ...
    private void drawLines() {
        ...
        Point prevPoint = convexHullPointList.get(0);
        for (int i = 1; i <= convexHullPointList.size() - 1; i++) {
            Point currentPoint = convexHullPointList.get(i);
            canvas.drawLine(prevPoint.x, prevPoint.y, currentPoint.x,
                    currentPoint.y, paint);
            prevPoint = currentPoint;
        }
        canvas.drawLine(prevPoint.x, prevPoint.y, convexHullPointList.get(0).x,
                convexHullPointList.get(0).y, paint);
        return;
    }
    ...
    public ArrayList<Point> getConvexHullPointList() {
